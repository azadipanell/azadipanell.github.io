...
- **پالت دستور** (`Ctrl/⌘+K` یا `/` یا دکمهٔ `⌘K` در ناوبر): جستجوی بخش‌ها، سؤال‌های متداول، مستندات و دستورها (تم، زبان، کپی لینک، اجرای دمو، …) — در هر دو نسخهٔ فارسی و انگلیسی
- **نقطه‌های میان‌بر بخش‌ها** در دسکتاپ، **حلقهٔ پیشرفت اسکرول** روی دکمهٔ بازگشت به بالا، **جستجو در سؤالات متداول**
- **بزرگ‌نمایی اسکرین‌شات‌ها** (لایت‌باکس با قبلی/بعدی)، **لینک مستقیم هر بخش** (¶ کنار تیترها)، **چرخش نقل‌قول**، **تمام‌صفحه شدن دمو**، **دکمهٔ نصب PWA** (فقط وقتی مرورگر اجازه بدهد)، سال خودکار فوتر، امضای کنسول
- **حریم خصوصی:** اسکریپت Web Analytics با توکن نمایشی حذف شد (صفر بیکن/کوکی) + استایل چاپ تمیز (باز شدن خودکار جواب‌ها هنگام چاپ)
- همهٔ موارد بالا در یک لایهٔ append-only در انتهای `index.html` و `en/index.html` هستند؛ با حذف بلوک `v2.4` همه‌چیز به حالت قبل برمی‌گردد
